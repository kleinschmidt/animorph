...
%% updated version of post-test stimuli, with different dists...

[xy_post, dirs_post, origin, dists, thetas] = ...
    make_posttest_stimuli('stimuli/animal_disc_8_0foot_0thigh_stimuli_4to1.mat', ...
    'stimuli/animal_disc_8_0foot_0thigh_stimuli_posttest.mat', ...
    [1, 2.5, 4]);

%% generate array of all stimulus images

orientations = [-pi/1.4, -pi/2.5, -pi/4, 0, 0.3927];
pitches = (0:-pi/24:-pi/4) + -pi/48;

[im_arr, params] = make_animal_cdata_array(origin, dirs_post, xy_post, orientations, pitches, 300);

save('stimuli/posttest_image_array.mat', 'im_arr', 'params', 'orientations', 'pitches', 'origin', 'dirs_post', 'xy_post');

%% generate array of all training stimuli
stimuli = open('stimuli/animal_disc_8_0foot_0thigh_stimuli_4to1.mat');
[im_arr_train, params] = make_animal_cdata_array(stimuli.origin, ...
    [stimuli.dir1' stimuli.dir2'], ...
    [stimuli.xy_in'; stimuli.xy_out'], ...
    orientations, pitches, 300);

ground_truth = repmat([ones(length(stimuli.xy_in), 1); ...
    zeros(length(stimuli.xy_out), 1)], length(orientations) * length(pitches));

save('stimuli/learning_image_array.mat', 'im_arr_train', 'params', 'orientations', 'pitches', 'stimuli', 'ground_truth');
